...
export class AuthorsListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  openDeleteDialog(author: Author) {
    const dialogRef = this.dialog.open(MessageComponent, {
      data: 'Deseja mesmo excluir esse autor?',
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.authorsService
          ...
          .pipe(
            catchError((err) => {
              this.messageService.error('Autor não pode ser excluido!');
              return err;
            })
          )
          .subscribe(() => {
            this.paginator.firstPage();
            this.refresh.next(true);
            this.messageService.success('Autor excluído com sucesso!');
          });
      }
    });
  }
  ...
  authorInfo(author: Author) {
    this.router.navigate([`/authors/author-info/${author.id}`]);
  }
}
